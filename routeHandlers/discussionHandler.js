...
exports.handleDiscussion = async (req, res) => {

    let content = (await fs.readFile('templates/discussion.ballz')).toString();

    let forumContent = '';
    for (let i = 0; i < posts.length; i++) {
        forumContent += `<article>
        <div>
        <h4>${posts[i].title}</h4>
        <p>${posts[i].author}</p>
        </div>
        ...
        <p>${posts[i].content}</p>
            <br>
            <time datetime="${posts[i].date}">${posts[i].date}</time>
        </article>`;
    }
    content = content.replace('%posts%', forumContent);
